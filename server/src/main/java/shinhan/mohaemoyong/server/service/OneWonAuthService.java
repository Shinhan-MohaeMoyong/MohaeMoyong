...
public class OneWonAuthService {
    ...
    @Transactional
    public void oneWonAuthCall(UserPrincipal userPrincipal, String accountNo) {
        // 1. 사용자 정보 조회
    ...
    @Transactional
    public void oneWonAuth(UserPrincipal userPrincipal, String accountNo, String authCode) {
        User user = userRepository.findById(userPrincipal.getId())
                .orElseThrow(() -> new IllegalArgumentException("[ERROR] 존재하지 않는 사용자입니다."));
        ...
        try {
            // 1. 외부 API 어댑터 호출
            response = demandDepositApiAdapter.oneWonAuth(user.getUserkey(), accountNo, authCode);
        } catch (ApiErrorException e) {
        ...
        if ("SUCCESS".equals(response.getREC().getStatus())) {
            log.info("계좌 인증에 성공했습니다. 계좌번호: {}", accountNo);

        } else {
            // 4. 인증 실패 시(예: status="FAIL"), 우리 서비스 명세에 맞는 예외 발생
            log.warn("인증 코드가 틀렸습니다. 계좌번호: {}", accountNo);
